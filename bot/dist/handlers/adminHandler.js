...
exports.handlePossibleOrderId = handlePossibleOrderId;
...
async function handleAdmin(bot, msg, callbackQuery) {
...
async function handleConfirmPaymentCallback(bot, chatId, orderId, userId) {
    try {
        const session = sessionManager_1.userSessions.getSession(userId || chatId.toString());
        if (!session?.token) {
            await bot.sendMessage(chatId, '❌ Вы не авторизованы.');
            return;
        }
        const loading = await bot.sendMessage(chatId, '⏳ Подтверждаю оплату...');
        await apiService_1.apiService.confirmPayment(orderId, session.token);
...
async function handleConfirmRejectCallback(bot, chatId, orderId, reason, userId) {
    try {
        const session = sessionManager_1.userSessions.getSession(userId || chatId.toString());
        if (!session?.token) {
            await bot.sendMessage(chatId, '❌ Вы не авторизованы.');
        ...
        await apiService_1.apiService.rejectOrder(orderId, reason, session.token);
...
async function handleAdminCommand(bot, chatId, command, args = [], userId) {
    try {
        const session = sessionManager_1.userSessions.getSession(userId || chatId.toString());
        ...
        logger_1.logger.error('Error handling admin command:', error);
        await bot.sendMessage(chatId, '❌ Произошла ошибка при выполнении команды');
    }
}
async function showAdminHelp(bot, chatId, session) {
    const availableCommands = adminCommands.filter(cmd => cmd.roles.includes(session.role));
...
async function handleOrdersList(bot, chatId, args, session) {
    try {
        const status = args[0] || 'PENDING_ADMIN';
        const ordersList = await apiService_1.apiService.getOrders(session.token, {
            status,
            limit: 10,
            sortBy: 'createdAt',
            sortOrder: 'desc'
        ...
        logger_1.logger.error('Error fetching orders list:', error);
        await bot.sendMessage(chatId, '❌ Ошибка при загрузке списка заказов');
    }
}
async function handleVerifyPayment(bot, chatId, args, session) {
    ...
    try {
        ...
        const keyboard = {
            inline_keyboard: orders.map((order, index) => [{
        ...
        logger_1.logger.error('Error in verify payment:', error);
        await bot.sendMessage(chatId, '❌ Ошибка при поиске заказа');
    }
}
async function handleRejectPayment(bot, chatId, args, session) {
    const orderId = args[0];
    ...
    try {
        ...
        logger_1.logger.error('Error in reject payment:', error);
        await bot.sendMessage(chatId, '❌ Ошибка при поиске заказа');
    }
}
async function handleAnalytics(bot, chatId, args, session) {
    try {
        const period = args[0] || '7d';
        const analytics = await apiService_1.apiService.getDashboardAnalytics(session.token, { period });
        if (!analytics.data) {
...
async function handleStoresList(bot, chatId, args, session) {
    try {
        const storesResponse = await apiService_1.apiService.getStores(session.token);
        ...
        const keyboard = {
            inline_keyboard: stores.slice(0, 5).map((store) => [{
                    text: `📊 ${store.name}`,
        ...
        logger_1.logger.error('Error fetching stores:', error);
        await bot.sendMessage(chatId, '❌ Ошибка при загрузке магазинов');
    }
}
async function handleInventoryAlerts(bot, chatId, args, session) {
    try {
        const severity = args[0]?.toUpperCase() || 'HIGH';
        const alerts = await apiService_1.apiService.getInventoryAlerts(session.token, { severity, limit: 10 });
        ...
        alerts.alerts.slice(0, 5).forEach((alert, index) => {
        ...
        logger_1.logger.error('Error fetching inventory alerts:', error);
        await bot.sendMessage(chatId, '❌ Ошибка при загрузке предупреждений');
    }
}
async function showOrderVerification(bot, chatId, order, session) {
    const orderNumber = order.orderNumber.slice(-8);
    const customer = order.customer?.firstName || order.customer?.username || 'Неизвестно';
    ...
    if (order.paymentProof) {
        try {
            ...
            logger_1.logger.error('Error sending payment proof photo:', photoError);
        }
    }
    await bot.sendMessage(chatId, message, {
        parse_mode: 'Markdown',
...
async function showOrderRejection(bot, chatId, order, reason, session) {
    const orderNumber = order.orderNumber.slice(-8);
...
async function handlePossibleOrderId(bot, chatId, text) {
    ...
    try {
        ...
        logger_1.logger.error('Error checking possible order ID:', error);
        return false;
    }
}
