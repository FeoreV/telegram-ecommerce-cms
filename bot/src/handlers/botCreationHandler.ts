...
export async function handleBotCreationCallback(bot: TelegramBot, chatId: number, callbackData: string) {
  ...
  try {
    switch (callbackData) {
      ...
      default:
        // Handle bot selection for creation
    ...
    logger.error('Bot creation callback error:', error);
...
async function startBotCreation(bot: TelegramBot, chatId: number, session: any) {
  // Check permissions
...
• Или удалите существующего бота, чтобы создать нового
    ...
    logger.error('Error loading stores for bot creation:', error);
    await bot.sendMessage(chatId, '❌ Ошибка при загрузке магазинов. Попробуйте позже.');
  }
}

async function showBotCreationForm(bot: TelegramBot, chatId: number, session: any, storeId: string) {
  try {
    // Get store info
    const storesResponse = await apiService.getUserStores(session.token);
    const stores = (storesResponse?.stores ?? []) as any[];
...
📨 *Отправьте токен следующим сообщением:*
    ...
    logger.error('Error showing bot creation form:', error);
    await bot.sendMessage(chatId, '❌ Ошибка при загрузке формы создания бота.');
  }
}

export async function handleBotCreationMessage(bot: TelegramBot, msg: TelegramBot.Message) {
  ...
  try {
    ...
    logger.error('Bot creation message error:', error);
    await bot.sendMessage(chatId, '❌ Произошла ошибка. Попробуйте еще раз.');
  }
}

async function handleTokenInput(bot: TelegramBot, chatId: number, session: any, token: string) {
...
async function handleUsernameInput(bot: TelegramBot, chatId: number, session: any, username: string) {
...
export async function handleBotCreationSkipCallback(bot: TelegramBot, chatId: number, callbackData: string) {
  const userId = chatId.toString();
  const session = userSessions.getSession(userId);
...
async function createBotFromSession(bot: TelegramBot, chatId: number, session: any) {
  ...
  try {
    const response = await apiService.createStoreBot({
      storeId: botCreationData.storeId,
      botToken: botCreationData.token,
      botUsername: botCreationData.username
    }, session.token);

    // Clear bot creation session
    userSessions.updateSession(session.telegramId, { botCreation: null });
...
https://t.me/${response.bot.botUsername}
    ...
    logger.info(`Bot created via Telegram: ${response.bot.botUsername} for store ${botCreationData.storeId} by user ${session.userId}`);
...
async function showBotList(bot: TelegramBot, chatId: number, session: any) {
  try {
...
*Создайте первого бота для вашего магазина!*
    ...
    logger.error('Error loading bot list:', error);
    await bot.sendMessage(chatId, '❌ Ошибка при загрузке списка ботов.');
  }
}

async function handleBotManagement(bot: TelegramBot, chatId: number, callbackData: string, session: any): Promise<boolean> {
...
async function showBotManagementMenu(bot: TelegramBot, chatId: number, session: any, storeId: string) {
  try {
    const response = await apiService.getUserBots(session.token);
    const bots = response.bots || [];
...
*Действия:*
    ...
    logger.error('Error showing bot management menu:', error);
...
async function restartBot(bot: TelegramBot, chatId: number, session: any, storeId: string) {
  ...
  try {
    await apiService.restartBot(storeId, session.token);
...
async function deleteBot(bot: TelegramBot, chatId: number, session: any, storeId: string) {
  const loadingMsg = await bot.sendMessage(chatId, '🗑️ Удаляю бота...');

  try {
    await apiService.deleteBot(storeId, session.token);
...
async function showMainMenu(bot: TelegramBot, chatId: number, session: any) {
  const text = `
🎛️ *Главное меню*
