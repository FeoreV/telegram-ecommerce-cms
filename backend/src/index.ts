...
// Ensure a stable per-client session identifier cookie for CSRF (avoids IP-based instability)
app.use((req, res, next) => {
  try {
    ...
    if (!req.cookies?.[name]) {
      ...
      // Make it available immediately for this request lifecycle
      req.cookies[name] = sid;
    }
  } catch (e) {
    logger.warn('Failed to ensure CSRF session id cookie', { error: (e as Error).message });
  }
  next();
});
...
const csrfProtection = doubleCsrf({
  ...
  getSessionIdentifier: (req) => {
    // Use stable per-client session identifier cookie to avoid IP-based mismatches
