 ...
 */
export const handleInviteLinkPage = asyncHandler(async (req: Request, res: Response) => {
  const { token } = req.params;

  if (!token) {
    throw new AppError('Токен инвайт ссылки не найден', 400);
  }

  try {
    // Получаем информацию об инвайт ссылке
...
}>, roleDisplay: string, roleColor: string, roleIcon: string, permissions: string[]): string {
  return `
    <!DOCTYPE html>
    <html lang="ru">
    <head>
