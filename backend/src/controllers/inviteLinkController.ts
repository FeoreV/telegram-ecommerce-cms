import { Response } from 'express';
import { AuthenticatedRequest } from '../middleware/auth';
import { prisma } from '../lib/prisma';
import { AppError, asyncHandler } from '../middleware/errorHandler';
import { logger } from '../utils/logger';
import {
  CreateInviteLinkSchema,
  UpdateInviteLinkSchema,
  InviteLinkSearchSchema,
  UseInviteLinkSchema
} from '../schemas/inviteLinkSchemas';
import { randomBytes } from 'crypto';
import { Prisma } from '@prisma/client';

/**
 * Создать инвайт ссылку
 */
export const createInviteLink = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const data = CreateInviteLinkSchema.parse(req.body);
  const { storeId, role, customRoleId, permissions, maxUses, expiresAt, description } = data;

  // Проверка доступа к магазину
  const hasAccess = await checkStoreAccess(req.user.id, storeId, req.user.role);
  if (!hasAccess) {
    throw new AppError('Нет доступа к этому магазину', 403);
  }

  // Проверяем, что кастомная роль принадлежит этому магазину
  if (customRoleId) {
    const customRole = await prisma.customRole.findFirst({
      where: {
        id: customRoleId,
        storeId,
        isActive: true
      }
    });

    if (!customRole) {
      throw new AppError('Кастомная роль не найдена', 404);
    }
  }

  const token = randomBytes(32).toString('hex');
  const expiresAtDate = expiresAt ? new Date(expiresAt) : null;

  const inviteLink = await prisma.inviteLink.create({
    data: {
      storeId,
      createdBy: req.user.id,
      token,
      role,
      customRoleId,
      permissions: permissions ? JSON.stringify(permissions) : null,
      maxUses,
      expiresAt: expiresAtDate,
      description
    },
    include: {
      store: {
        select: {
          id: true,
          name: true
        }
      },
      customRole: {
        select: {
          id: true,
          name: true,
          color: true,
          icon: true
        }
      }
    }
  });

  logger.info('Invite link created', {
    inviteLinkId: inviteLink.id,
    storeId,
    createdBy: req.user.id,
    role: role || 'CUSTOM',
    customRoleId
  });

  res.status(201).json({
    success: true,
    inviteLink: {
      ...inviteLink,
      url: `${process.env.FRONTEND_URL}/invite/${token}`
    }
  });
});

/**
 * Получить список инвайт ссылок
 */
export const getInviteLinks = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const searchParams = InviteLinkSearchSchema.parse(req.query);
  const { storeId, isActive, page, limit } = searchParams;

  const whereClause: Prisma.InviteLinkWhereInput = {};

  if (storeId) {
    // Проверка доступа к магазину
    const hasAccess = await checkStoreAccess(req.user.id, storeId, req.user.role);
    if (!hasAccess) {
      throw new AppError('Нет доступа к этому магазину', 403);
    }
    whereClause.storeId = storeId;
  } else {
    // Если storeId не указан, показываем ссылки для всех доступных магазинов
    if (req.user.role !== 'OWNER') {
      const accessibleStores = await getAccessibleStores(req.user.id, req.user.role);
      whereClause.storeId = { in: accessibleStores };
    }
  }

  if (isActive !== undefined) {
    whereClause.isActive = isActive;
  }

  const skip = (page - 1) * limit;

  const [inviteLinks, total] = await Promise.all([
    prisma.inviteLink.findMany({
      where: whereClause,
      include: {
        store: {
          select: {
            id: true,
            name: true
          }
        },
        customRole: {
          select: {
            id: true,
            name: true,
            color: true,
            icon: true
          }
        },
        creator: {
          select: {
            firstName: true,
            lastName: true,
            username: true
          }
        },
        _count: {
          select: {
            invitations: true
          }
        }
      },
      orderBy: {
        createdAt: 'desc'
      },
      skip,
      take: limit
    }),
    prisma.inviteLink.count({ where: whereClause })
  ]);

  // Форматируем данные
  const formattedLinks = inviteLinks.map(link => ({
    ...link,
    url: `${process.env.FRONTEND_URL}/invite/${link.token}`,
    usageStats: {
      used: link.usedCount,
      max: link.maxUses,
      remaining: link.maxUses - link.usedCount
    },
    isExpired: link.expiresAt ? link.expiresAt < new Date() : false,
    invitationsCount: link._count.invitations
  }));

  res.json({
    inviteLinks: formattedLinks,
    pagination: {
      page,
      limit,
      total,
      totalPages: Math.ceil(total / limit)
    }
  });
});

/**
 * Обновить инвайт ссылку
 */
export const updateInviteLink = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const data = UpdateInviteLinkSchema.parse(req.body);
  const { id, isActive, maxUses, expiresAt, description } = data;

  const inviteLink = await prisma.inviteLink.findUnique({
    where: { id },
    include: { store: true }
  });

  if (!inviteLink) {
    throw new AppError('Инвайт ссылка не найдена', 404);
  }

  // Проверка доступа
  const hasAccess = await checkStoreAccess(req.user.id, inviteLink.storeId, req.user.role);
  if (!hasAccess) {
    throw new AppError('Нет доступа к этой ссылке', 403);
  }

  const updateData: Prisma.InviteLinkUpdateInput = {};
  if (isActive !== undefined) updateData.isActive = isActive;
  if (maxUses !== undefined) updateData.maxUses = maxUses;
  if (expiresAt !== undefined) updateData.expiresAt = new Date(expiresAt);
  if (description !== undefined) updateData.description = description;

  const updatedLink = await prisma.inviteLink.update({
    where: { id },
    data: updateData,
    include: {
      store: {
        select: {
          id: true,
          name: true
        }
      },
      customRole: {
        select: {
          id: true,
          name: true,
          color: true,
          icon: true
        }
      }
    }
  });

  logger.info('Invite link updated', {
    inviteLinkId: id,
    updatedBy: req.user.id,
    changes: updateData
  });

  res.json({
    success: true,
    inviteLink: {
      ...updatedLink,
      url: `${process.env.FRONTEND_URL}/invite/${updatedLink.token}`
    }
  });
});

/**
 * Удалить инвайт ссылку
 */
export const deleteInviteLink = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;

  const inviteLink = await prisma.inviteLink.findUnique({
    where: { id }
  });

  if (!inviteLink) {
    throw new AppError('Инвайт ссылка не найдена', 404);
  }

  // Проверка доступа
  const hasAccess = await checkStoreAccess(req.user.id, inviteLink.storeId, req.user.role);
  if (!hasAccess) {
    throw new AppError('Нет доступа к этой ссылке', 403);
  }

  await prisma.inviteLink.delete({
    where: { id }
  });

  logger.info('Invite link deleted', {
    inviteLinkId: id,
    deletedBy: req.user.id
  });

  res.json({
    success: true,
    message: 'Инвайт ссылка удалена'
  });
});

/**
 * Получить информацию об инвайт ссылке (публичный метод)
 */
export const getInviteLinkInfo = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { token } = (req as any).params;

  const inviteLink = await prisma.inviteLink.findUnique({
    where: { token },
    include: {
      store: {
        select: {
          id: true,
          name: true,
          description: true,
          logoUrl: true
        }
      },
      customRole: {
        select: {
          id: true,
          name: true,
          description: true,
          permissions: true,
          color: true,
          icon: true
        }
      },
      creator: {
        select: {
          firstName: true,
          lastName: true,
          username: true
        }
      }
    }
  });

  if (!inviteLink) {
    throw new AppError('Инвайт ссылка не найдена', 404);
  }

  if (!inviteLink.isActive) {
    throw new AppError('Инвайт ссылка деактивирована', 400);
  }

  if (inviteLink.expiresAt && inviteLink.expiresAt < new Date()) {
    throw new AppError('Срок действия инвайт ссылки истек', 400);
  }

  if (inviteLink.usedCount >= inviteLink.maxUses) {
    throw new AppError('Лимит использований инвайт ссылки исчерпан', 400);
  }

  // Определяем разрешения
  let permissions: string[] = [];
  if (inviteLink.customRole) {
    permissions = JSON.parse(inviteLink.customRole.permissions) as string[];
  } else if (inviteLink.permissions) {
    permissions = JSON.parse(inviteLink.permissions) as string[];
  }

  res.json({
    inviteLink: {
      id: inviteLink.id,
      role: inviteLink.role,
      customRole: inviteLink.customRole,
      permissions,
      description: inviteLink.description,
      expiresAt: inviteLink.expiresAt,
      store: inviteLink.store,
      creator: inviteLink.creator,
      usageStats: {
        used: inviteLink.usedCount,
        max: inviteLink.maxUses,
        remaining: inviteLink.maxUses - inviteLink.usedCount
      }
    }
  });
});

/**
 * Использовать инвайт ссылку для регистрации
 */
export const useInviteLink = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const data = UseInviteLinkSchema.parse(req.body);
  const { token, firstName, lastName, email, telegramId } = data;

  const inviteLink = await prisma.inviteLink.findUnique({
    where: { token },
    include: {
      store: true,
      customRole: true
    }
  });

  if (!inviteLink) {
    throw new AppError('Инвайт ссылка не найдена', 404);
  }

  if (!inviteLink.isActive) {
    throw new AppError('Инвайт ссылка деактивирована', 400);
  }

  if (inviteLink.expiresAt && inviteLink.expiresAt < new Date()) {
    throw new AppError('Срок действия инвайт ссылки истек', 400);
  }

  if (inviteLink.usedCount >= inviteLink.maxUses) {
    throw new AppError('Лимит использований инвайт ссылки исчерпан', 400);
  }

  // Проверяем, существует ли пользователь
  let user = await prisma.user.findUnique({
    where: { email }
  });

  await prisma.$transaction(async (tx) => {
    // Создаем или обновляем пользователя
    if (!user) {
      user = await tx.user.create({
        data: {
          telegramId: telegramId || `invite_${randomBytes(8).toString('hex')}`,
          firstName,
          lastName,
          email,
          role: 'CUSTOMER', // Временная роль
          isActive: true
        }
      });
    } else {
      // Обновляем данные существующего пользователя
      user = await tx.user.update({
        where: { id: user.id },
        data: {
          firstName,
          lastName,
          ...(telegramId && { telegramId }),
          isActive: true
        }
      });
    }

    // Создаем назначение роли
    if (inviteLink.role === 'ADMIN') {
      await tx.storeAdmin.upsert({
        where: {
          storeId_userId: {
            storeId: inviteLink.storeId,
            userId: user.id
          }
        },
        create: {
          userId: user.id,
          storeId: inviteLink.storeId,
          assignedBy: inviteLink.createdBy
        },
        update: {}
      });
    } else {
      await tx.storeVendor.upsert({
        where: {
          storeId_userId: {
            storeId: inviteLink.storeId,
            userId: user.id
          }
        },
        create: {
          userId: user.id,
          storeId: inviteLink.storeId,
          assignedBy: inviteLink.createdBy,
          isActive: true,
          customRoleId: inviteLink.customRoleId,
          permissions: inviteLink.permissions
        },
        update: {
          customRoleId: inviteLink.customRoleId,
          permissions: inviteLink.permissions,
          isActive: true
        }
      });
    }

    // Увеличиваем счетчик использований
    await tx.inviteLink.update({
      where: { id: inviteLink.id },
      data: {
        usedCount: { increment: 1 }
      }
    });

    // Создаем запись активности
    await tx.employeeActivity.create({
      data: {
        userId: user.id,
        storeId: inviteLink.storeId,
        action: 'INVITE_LINK_USED',
        details: JSON.stringify({
          inviteLinkId: inviteLink.id,
          role: inviteLink.role || 'CUSTOM',
          customRoleId: inviteLink.customRoleId
        })
      }
    });
  });

  logger.info('Invite link used', {
    inviteLinkId: inviteLink.id,
    userId: user.id,
    email,
    storeId: inviteLink.storeId
  });

  res.json({
    success: true,
    message: 'Добро пожаловать в команду!',
    user: {
      id: user.id,
      firstName: user.firstName,
      lastName: user.lastName,
      email: user.email
    },
    store: {
      id: inviteLink.store.id,
      name: inviteLink.store.name
    },
    role: inviteLink.customRole ? inviteLink.customRole.name : inviteLink.role
  });
});

// Вспомогательные функции
async function checkStoreAccess(userId: string, storeId: string, role: string): Promise<boolean> {
  if (role === 'OWNER') return true;

  const store = await prisma.store.findFirst({
    where: {
      id: storeId,
      OR: [
        { ownerId: userId },
        { admins: { some: { userId } } }
      ]
    }
  });

  return !!store;
}

async function getAccessibleStores(userId: string, role: string): Promise<string[]> {
  if (role === 'OWNER') {
    const stores = await prisma.store.findMany({
      select: { id: true }
    });
    return stores.map(store => store.id);
  }

  const stores = await prisma.store.findMany({
    where: {
      OR: [
        { ownerId: userId },
        { admins: { some: { userId } } }
      ]
    },
    select: { id: true }
  });

  return stores.map(store => store.id);
}
