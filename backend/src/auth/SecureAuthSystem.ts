...
import { getAuthConfig, shouldRefreshToken, parseExpiryToSeconds } from './AuthConfig';
import type { UserWithPermissions } from '../types/express.d';

// Security Configuration
const JWT_SECRET = process.env.JWT_SECRET || (() => {
  logger.error('JWT_SECRET not set! Generating temporary secret for development');
  return crypto.randomBytes(64).toString('hex');
})();

const JWT_REFRESH_SECRET = process.env.JWT_REFRESH_SECRET || (() => {
  logger.error('JWT_REFRESH_SECRET not set! Generating temporary secret for development');
  return crypto.randomBytes(64).toString('hex');
})();
...
export class SecureAuthSystem {
  ...
  static generateRefreshToken(payload: Omit<RefreshTokenPayload, 'iat' | 'exp' | 'tokenType'>): string {
    ...
    return jwt.sign(tokenPayload, JWT_REFRESH_SECRET as string, options);
  }
