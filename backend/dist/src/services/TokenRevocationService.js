...
class TokenRevocationService {
    ...
    async isTokenRevoked(tokenId) {
        try {
            ...
            const cached = await TenantCacheService_1.tenantCacheService.exists('system', `revoked_${tokenId}`, { namespace: 'revocation' });
            if (cached) {
                this.memoryRevocationList.add(tokenId);
                return true;
            }
      ...
      `;
            ...
            if (isRevoked) {
                ...
                if (revocationData) {
                    ...
                    if (ttl > 0) {
                        await TenantCacheService_1.tenantCacheService.set('system', `revoked_${tokenId}`, revocationData, {
                            ttl,
                            namespace: 'revocation'
                        });
                    }
