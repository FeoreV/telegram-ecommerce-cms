...
async function main() {
    ...
    for (let i = 0; i < 15; i++) {
        ...
        if (randomStatus === 'PAID' || randomStatus === 'SHIPPED' || randomStatus === 'DELIVERED') {
            paidAt = new Date(orderDate.getTime() + Math.random() * 24 * 60 * 60 * 1000);
        }
        if (randomStatus === 'SHIPPED' || randomStatus === 'DELIVERED') {
            shippedAt = new Date(paidAt.getTime() + Math.random() * 48 * 60 * 60 * 1000);
        }
        if (randomStatus === 'DELIVERED') {
            deliveredAt = new Date(shippedAt.getTime() + Math.random() * 72 * 60 * 60 * 1000);
        }
